...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Exercise 5.1", NULL, NULL);
    if (window == NULL)
    {
...
glm::mat4 viewProjection(){
    ...
    if (g_perspectiveProjection) {
        ...
        glm::vec3 centerPosition = cameraPosition + forwardDirection;
        glm::mat4 viewMatrix = glm::lookAt(cameraPosition, centerPosition, upVector);
        ...
        glm::mat4 projectionMatrix = glm::perspectiveFov(
                fieldOfView,(float)screenWidth, (float)screenHeight, nearPlane, farPlane);
