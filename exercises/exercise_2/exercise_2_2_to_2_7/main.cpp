...
void bindAttributes(){
    ...
    GLCall(glVertexAttribPointer(vertexLocation, posSize, GL_FLOAT, GL_FALSE, particleSize * sizeOfFloat, 0));
    ...
    GLCall(glEnableVertexAttribArray(vertexVelocity));
    GLCall(glVertexAttribPointer(vertexVelocity, velocitySize, GL_FLOAT, GL_FALSE,
                          particleSize*sizeof(float), (void*)(posSize * sizeof(float))));

    int timeOfBirthSize = 1;
    GLuint vertexBirth = glGetAttribLocation(shaderProgram->ID, "timeOfBirth");
    GLCall(glEnableVertexAttribArray(vertexBirth));
    GLCall(glVertexAttribPointer(vertexBirth, timeOfBirthSize, GL_FLOAT, GL_FALSE,
                          particleSize*sizeof(float), (void*)((posSize+velocitySize)*sizeof(float))));
