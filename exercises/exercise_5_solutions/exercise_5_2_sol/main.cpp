...
float cameraMoveSpeed = 0.15f, rotationGain = 30.0f;
...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Exercise 5.2", NULL, NULL);
    if (window == NULL)
    {
...
void drawObjects(){
    ...
    glm::mat4 projection = glm::perspectiveFov(70.0f, (float)screenWidth, (float)screenHeight, .01f, 100.0f);
...
void processInput(GLFWwindow *window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS){
        camPosition += forwardInXZ * cameraMoveSpeed;
    }
    ...
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS){
        ...
        camPosition -= glm::cross(forwardInXZ, glm::vec3(0, 1, 0)) * cameraMoveSpeed;
    }
